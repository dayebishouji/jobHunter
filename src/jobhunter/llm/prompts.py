...
EXTRACT_COMPANY_PROFILE_SUFFIX = (
    "\n\n本次目标：抽取该公司的**公司画像 / 基本面**——"
    "（A）description: 一句话中文简介（≤50 字），"
    "（B）official_website: 官网 URL，"
    "（C）main_business: 主营业务/经营范围概括（list[str]，最多 3 条），"
    "（D）products: 主要产品/产品线（list[str]），"
    "（E）industries: 所属行业（list[str]），"
    "（F）company_size: 员工规模区间字符串（如 '100-500人' / '5000-10000人'），"
    "（G）employee_count: 实际员工人数（int，如 1200；原文是 '约 1200 人' 也按 1200 给），"
    "（H）insured_count: **社保参保人数**（int）—— 这是判断公司真实规模的最强信号，"
    "    来源通常是工商档案 / 天眼查 / 企查查 / 爱企查；"
    "    找不到 → null（不要凭空编），"
    "（I）founded_year: 成立年份（int，如 2015），"
    "（J）funding_stage: 融资阶段（天使 / A轮 / B轮 / C轮 / D轮及以上 / 已上市 / 未融资），"
    "（K）total_funding: 累计融资额（如 '约 5 亿元'），"
    "（L）investors: 主要投资方（list[str]，最多 5 个），"
    "（M）headquarters: 总部所在地（如 '杭州'），"
    "（N）prospects: 发展前景概述（≤80 字，基于新闻 + 财报 + 公开战略），"
    "（O）source_urls: 抽取依据 URL 列表（list[HttpUrl]）。"
    "\n\n**重要约束**："
    "- 所有字段原文未提 → null，不要瞎编。"
    "- employee_count 与 insured_count **必须区分**：前者是当前规模，后者是社保实缴。"
    "- 找不到精确数字 → 给区间字符串（company_size）就够了，不要瞎给整数。"
)
...
EXTRACT_REVIEWS_SUFFIX = (
    ...
    "（H）v0.1.21 — 典型下班时间 typical_off_time: UGC 里反复出现的「团队典型下班时间」"
    "（如 '约 10:00 PM' / '弹性 9-6' / '21 点左右' / '7 点准时'），"
    "配 typical_off_time_evidence（一句话原文引用，≤30 字）和 typical_off_time_url（最相关的 1 个 url）。"
    "原文没提具体时间 → 全部留空，不要给 null/未填。"
