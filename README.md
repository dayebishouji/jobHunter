...
- **v0.3.5 hotfix #2**（用户 江苏苏海信息科技 实测反馈）修 v0.3.5 B 层 `QnaReviewsCollector` 静默软失败 bug：`search/tavily_client.py:129` 写 `result.get("answer")`，但 Tavily SDK 的 `AsyncTavilyClient.qna_search` 实际返回 `str`（已用 `inspect.signature` 验证），每次跑都抛 `AttributeError: 'str' object has no attribute 'get'`，被软失败吞掉 → Layer B AI 摘要信号源整个丢失。修：去掉 `.get("answer")` 直接 strip 返回（1 行）；`tests/test_v035_max_recall.py` 新增 `TestTavilyClientQnaSearchWrapper` 4 cases pin SDK 契约。**教训**：v0.3.5 自测没抓到是因为 `TestQnaReviewsCollector` 用 `_FakeTavily(answer="...")` mock 掉了整个 wrapper，没走真实 `TavilyClient` —— 测试 mock 要按 SDK 真实返回类型写。596 pass（前 592 + 4 新增）。零 LLM 调用增量，零 schema 改动
